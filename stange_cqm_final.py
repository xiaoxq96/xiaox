...
class eindim_Problem():
    ...
    def define_variables(self):
        for i in range(self.num_stueck):
            self.x ={i:Integer(f'x_{i}',
                         lower_bound=0,
                         upper_bound=self.num_stange*self.stange_lange)
                     for i in range(self.num_stueck)}
    ...
    def grenze_constraint(self):
        ...
        for i in range(self.num_stueck):
            ...
            for j in range(self.num_stange):
                ...
                '''
                self.cqm.add_constraint((self.x[(i)] + self.stueck_lange[(i)]) - (j+1)*self.stange_lange-
                                        ((1-self.p[(i,j)])*self.gesamte_stange_lange)<= 0,
                                        label=f'maxx_{i}_{j}_less')
                self.cqm.add_constraint((
                    self.x[(i)] - self.stange_lange * j * self.p[(i, j)]) >= 0,
                    label=f'maxx_{i}_{j}_greater')
    ...
    def geomerie_constraint(self):
        ...
        for i, k in combinations(range(self.num_stueck),r=2):
            ...
            for j in range(self.num_stange):
                ...
                self.cqm.add_constraint((self.x[(i)]+self.stueck_lange[(i)]-self.x[(k)])-(
                                            (2-self.ik-
                                             self.b[i,k,0])*
                                            self.num_stange*self.stange_lange) <= 0,
                                            label=f'overlap_{i}_{k}_{j}_0')
                self.cqm.add_constraint((self.x[(k)]+self.stueck_lange[(k)]-self.x[(i)])-(
                                            (2-self.ik-
                                             self.b[i,k,1])*
                                            self.num_stange*self.stange_lange) <= 0,
    ...
    def stell_obejektive(self,weight_1):
        obj_1= quicksum(self.s[(j)]for j in range(self.num_stange))
        self.cqm.set_objective(weight_1*obj_1)
        return
                    
    def call_bqm_solver(self):
        sampler = LeapHybridCQMSampler()
        raw_sampleset = sampler.sample_cqm(self.cqm,label="1d_cqm_9",time_limit=10)    #Optimal solution with time_limit 
    ...
    def zeichnung(self):
        ...
        x_rest=[]
        for i in range(self.num_stueck):
            x_rest.append(self.x_solution[f'x_{i}'] % self.stange_lange)
        print(sum(x_rest))
