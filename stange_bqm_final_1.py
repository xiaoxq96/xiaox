...
class eindim_Problem():
    ...
    def stuecke_position_constraint(self,weight):
        ...
        for i in range(self.num_stueck):
            for j in range(self.num_stange):
                for a in range(j*self.stange_lange,(j+1)*self.stange_lange-self.stueck_lange[(i)]+1):
                            self.bqm.add_quadratic(self.S_j[(j)],self.X_ija[(i,j,a)],weight)
        '''
        for j in range(self.num_stange):
            for i in range(self.num_stueck):
    ...
    def position_objektive(self,weight):
        ...
        for i in range(self.num_stueck):
            for j in range(self.num_stange):
                for a in range(j*self.stange_lange,(j+1)*self.stange_lange-self.stueck_lange[(i)]+1):
                    bias[self.X_ija[(i,j,a)]]=a*weight
    ...
    def anzhal_objektive(self,weight):
        #所用面积最小
        for j in range(self.num_stange):
            self.bqm.add_linear(self.S_j[(j)],-weight)
        return
...
if __name__=="__main__":
    ...
    '''
    starttime=datetime.datetime.now()
    solution,qpu_access_time= a.call_bqm_solver(max_iter=2,convergence=1)
    endtime=datetime.datetime.now()
    print ("starttime:",starttime)
    print ("endtime:",endtime)
    ...
    '''
    starttime=datetime.datetime.now()
    solution= a.klassische_solve(max_iter=5,num_reads=1500,convergence=3)
    endtime=datetime.datetime.now()
    print ("starttime:",starttime)
    print ("endtime:",endtime)
